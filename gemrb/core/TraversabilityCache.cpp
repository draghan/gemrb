...
void TraversabilityCache::UpdateActorPosition(const Actor *actor, const NavmapPoint &OldPos, const NavmapPoint &NewPos, int inWidth) {
	...
	// clear old position
	const auto oldRegion = CalculateRegion(actor, OldPos);
	const auto NewRegion = CalculateRegion(actor, NewPos);

	// std::string ClearedS;
	for (int x = std::max(0, oldRegion.origin.x); x < oldRegion.origin.x + oldRegion.size.w; ++x) {
		for (int y = std::max(0, oldRegion.origin.y); y < oldRegion.origin.y + oldRegion.size.h; ++y) {
			const auto Idx = y * inWidth * 16 + x;
			if (traversabilityData[Idx].occupyingActor == actor) {
				traversabilityData[Idx] = TraversabilityCellData {};
				// ClearedS += "x";
			}
			else {
				// ClearedS += "o";
	...
	// std::string MarkedS;
	for (int x = std::max(0, NewRegion.origin.x); x < NewRegion.origin.x + NewRegion.size.w; ++x) {
		for (int y = std::max(0, NewRegion.origin.y); y < NewRegion.origin.y + NewRegion.size.h; ++y) {
			if (!actor->IsOver(Point { x, y }, NewPos)) {
				// MarkedS += "o";
				continue;
			}
			// MarkedS += "x";
...
bool TraversabilityCache::ValidateTraversabilityCacheSize()
{
